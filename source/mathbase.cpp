...
//	判断一个数是否有原根，是 yes  否 no  报错 "# 错误信息"
std::string PrimaryRootIsVaild(int p) {
	//	如果 有原根  则 p 是 2 或 4 或 a的n次幂 或 2*（a的n次幂）	a为奇素数
	if (p == 2 || p == 4)	return "yes";
	//	进行质因数分解
	std::string factorStr = factorize(p);
	if (factorStr[0] == '#')	return factorStr;
	std::map <std::string, std::string> base_power = AnalyseFactorStr2(factorStr);
	//	因为必须是 a的n次幂 ，所以如果分解出了多个质因数，说明它不是 a的n次幂，直接返回 no 即可
	if (base_power.size() > 1)	return "no";
	//	如果只有一个因数，那么它已经是质数了，只需要判断是不是2即可
	for (auto const& pair : base_power) {
		if (pair.first == "2")	return "no";
	}
	return "yes";
}

//	求mod p的原根
std::string PrimaryRoot(int p) {
	if (p <= 1)	return "# P should be greater than 1 !";
	//	在这之前要 检查p
	//	1. 判断 p 是否有原根
	std::string isVaild = PrimaryRootIsVaild(p);
	if (isVaild[0] == '#')	return isVaild;
	else if (isVaild == "no")	return "# p has no primary root";
	//	2. 求 原根的个数 --- p-1的欧拉函数个
	std::string retNum = Eular(std::to_string(p - 1));

	//	3. 求 p-1 的所有质因子  qi
	std::string factorStr = factorize(p - 1);
	std::map <std::string, std::string> base_power = AnalyseFactorStr2(factorStr);
	...
	int count = 0;
	for (auto const& pair : base_power) {
		q[count] = (p - 1) / std::stoi(pair.first);
		++count;
	}
	int times = p - 1;

	//	5. 拿 2 3 5 一个一个尝试	得到一个原根 g
	int g;
	int tryNum = 2;
	while (tryNum < p) {
		//	进行尝试
		bool isTrue = true;		//	默认该数是原根
		for (int i = 0; i < count; ++i) {
			std::string power = N_power(std::to_string(tryNum), q[i]);
			//	(power-1) % p 
			std::string power_1 = subtraction(power, "1");
			std::string remainder = std::get<1>(division2(power_1, std::to_string(p)));
			if (remainder == "0") {
				//	和 1 同余了，不符合要求了
				isTrue = false;
				break;
		...
		bool ok = false;
		while (!ok) {
			//	tryNum每次 +1 的时候要进行检查, 因为不要 >=2的 幂次数
			++tryNum;
			//	检查的方法就是质因数分解，如果只有一个因数，且该因数的次数大于1，则跳过该数，直到找到一个原根 g
			std::string factors = factorize(tryNum);
			std::map<std::string, std::string> m = AnalyseFactorStr2(factors);
			if (m.size() > 1)		break;
			//	取出那一个因数和幂数
	...
	std::string gs;
	//	6. 由(d,p-1) = 1,得到 d 遍历 p-1 的完全剩余系，然后 g的d次幂 遍历 p的所有原根，保存即可 
	std::string p_1_str = std::to_string(p - 1);
	for (int d = 1; d < p - 1; ++d) {
		std::string d_str = std::to_string(d);
		std::string mcf = MaxCommonFactor(d_str, p_1_str);
		if (mcf[0] == '#')	return mcf;
		ValidNumStr(mcf);
		if (mcf != "1")		continue;
		//	开始制作原根
		std::string g1 = std::get<1>(division2(N_power(std::to_string(g), d), std::to_string(p)));
		gs += (g1 + " ");
	}
	delete[] q;
	return gs;
