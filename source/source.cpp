...
void ExecuteCommand(std::string command) {
	...
	if (FuncId == 0) {
		...
		std::string p_str = GetCommandArgs(0);
		std::string isMatch = CheckStrIsNumber(p_str);
		if (isMatch[0] == '#') {
			Red(isMatch);
			PauseExit(1);
		}
		ValidNumStr(p_str);
		bool f = IsGreatIntMax(p_str);
		if (f) {
			Red("# Hopefully, p will be less than 2147483647!\n");
			PauseExit(1);
		}
		...
		std::string gs = PrimaryRoot(std::stoi(p_str));
		std::any a = gs;
		(*p).AddResults(a);
	}
	else {
